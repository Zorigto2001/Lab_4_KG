...
const Matrix Surface::Scale(const Matrix &vertices)//приближение (Вычисление подуразмеров)
{
...
void Surface::Draw(Graphics &g)//прорисовывоние (Устанавливаем горизонты)
{
	...
	for (int32_t z = 0; z < steps_; z++)//(алгоритм плавающего горизонта) 4
	{
		...
		for (int32_t x = 0; x < steps_; x++)
		{
			...
			flag = y1 < upper[x1] || y1 > lower[x1];//тест видимости точек
